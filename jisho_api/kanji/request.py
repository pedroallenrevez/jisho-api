...
class KanjiRequest(BaseModel):
    ...
    def rich_print(self):
        ...
        base += CLITagger.colorize('Kun', ', '.join(self.data.main_readings.kun), 'red',)
        base += CLITagger.colorize('On', ', '.join(self.data.main_readings.on), 'red', last=True)
        console.print(base)
        # TODO - TRY on this
        ...
        try:
            ...
            base += CLITagger.colorize('Grade', self.data.meta.education.grade, 'magenta', last=True)
        except Exception as e:
            print(e)
        console.print(base)

        console.print(f"Radical no {self.data.radical.kangxi_order}:")
        console.print(CLITagger.colorize('Base', f"{self.data.radical.basis} - {self.data.radical.meaning}", 'yellow', last=True))
        try:
            console.print(CLITagger.colorize('Alternate Radical', ', '.join(self.data.radical.alt_forms), 'yellow', last=True))
        except Exception as e:
            pass
        console.print(CLITagger.colorize('Parts', ', '.join(self.data.radical.parts), 'yellow', last=True))
        try:
            console.print(CLITagger.colorize('Variants', ', '.join(self.data.radical.variants), 'yellow', last=True))
        except Exception as e:
            pass
        ...
        for m in self.data.reading_examples.on:
            bullet_text = f"{m.kanji} [yellow][{m.reading}] [white]{', '.join(m.meanings)}"
...
class Kanji:
    ...
    @staticmethod
    def strokes(soup):
        return soup.find_all("div", {"class": "kanji-details__stroke_count"})[0].find('strong').text

    @staticmethod
    def main_meanings(soup):
        return soup.find_all("div", {"class": "kanji-details__main-meanings"})[0].text.strip().split(', ')

    @staticmethod
    def main_readings(soup):
        res = soup.find_all("div", {"class": "kanji-details__main-readings"})
        try:
            kun = res[0].find_all("dl", {"class": "dictionary_entry kun_yomi"})[0].text.replace('\n', '').replace('Kun:', '').split('、 ')
        except: 
            kun = None

        try:
            on = res[0].find_all("dl", {"class": "dictionary_entry on_yomi"})[0].text.replace('\n', '').replace('On:', '').split('、 ')
        except:
            on = None
        return {
            "kun": kun, "on": on
    ...
    def meta_readings(soup):
        ...
        try:
            kr = res[0].find_all("dl", {"class": "dictionary_entry korean"})[0].find_all('dd')[0].text.split(', ')
        except:
            kr = None
    ...
    def meta_education(soup):
        ...
        try:
            grade = res.find_all('div', {"class": "grade"})[0].find_all("strong").text.split(' ')[-1]
        except:
            grade = None
        ...
        try:
            frequency = res.find_all('div', {"class": "frequency"})[0].find_all("strong").text
        except:
            frequency = None
    ...
    def reading_examples(soup):
        try:
            threeway = lambda x: (x[:x.index('【')], x[x.index('【')+1:x.index('】')], x[x.index('】')+1:])
            process = lambda x: [
                    {
                        "kanji": k.replace('\n', '').strip(),
                        "reading": r.replace('\n', '').strip(),
    ...
    def radical(soup):
        try:
            variants = soup.find_all("dl", {"class": "dictionary_entry variants"})[0].find('a').text.split(' ')
        except:
            variants = None
    ...
    def request(kanji, cache=False):
        ...
        try:
            ...
            console.print(f"[red bold ][Error][/red bold] [white]No kanji found with name {kanji}.")
            return None
